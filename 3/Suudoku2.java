...
public class Suudoku2 {

  public Suudoku2(int[][] data) {
    solve(data);
  }
  ...
  void c_Diagonal(boolean[] list, int[][] data) {
    for (int i = 0; i < 9; i++) if (data[i][i] != 0) list[data[i][i] - 1] =
      true;
  }

  void c_Diagonal_rev(boolean[] list, int[][] data) {
    for (int i = 0; i < 9; i++) if (data[i][8 - i] != 0) list[data[i][8 - i] -
      1] =
      true;
  }
  ...
  public void solve(int[][] data) {
    for (int i = 0; i < 9; i++) for (int j = 0; j < 9; j++) if (
      ...
      if (i == j) {
        c_Diagonal(list, data);
      }
      if (i + j == 8) {
        c_Diagonal_rev(list, data);
      }
